...
const deck_schema_1 = require("./schema/deck.schema");
...
let DeckService = class DeckService {
    ...
    async fetchCommander(commanderName) {
        const url = `https://api.scryfall.com/card/named?exact-${encodeURIComponent(commanderName)}`;
    ...
    async fetchCardsByColors(colors) {
        ...
        const response = await fetch(`https://api.scryfall.com/card/search?q=color%3D${colorQuery}&unique=cards&order=random`);
        const data = await response.json();
        const cards = [];
